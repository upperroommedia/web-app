...
} from '../utils/deleteSubsplashMediaAndLocalState';
import { runWithMissingSubsplashMediaRecovery } from '../utils/subsplashMediaRecovery';
...
export const resolveSessionSubsplashUploadGeneration = (
  sessionUploadGeneration?: number | null,
  sermonUploadGeneration?: number | null
): number => {
  if (typeof sessionUploadGeneration === 'number' && Number.isFinite(sessionUploadGeneration)) {
    return sessionUploadGeneration;
  }

  if (typeof sermonUploadGeneration === 'number' && Number.isFinite(sermonUploadGeneration)) {
    return sermonUploadGeneration;
  }
...
const SermonPublishPanel: FunctionComponent<SermonPublishPanelProps> = ({
  ...
  const [sessionSubsplashUploadGeneration, setSessionSubsplashUploadGeneration] = useState<number>(() =>
    resolveSessionSubsplashUploadGeneration(undefined, sermon.subsplashUploadGeneration)
  );
  ...
  useEffect(() => {
    setSessionSubsplashUploadGeneration(resolveSessionSubsplashUploadGeneration(undefined, sermon.subsplashUploadGeneration));
  }, [sermon.id, sermon.subsplashUploadGeneration]);
  ...
  const ensureSubsplashMediaItem = useCallback(async (
    options?: { forceCreate?: boolean }
  ): Promise<SubsplashMediaItemResult> => {
    const existingMediaItemId = resolveSessionSubsplashMediaItemId(sessionSubsplashMediaItemId, sermon.subsplashId);
    if (existingMediaItemId && !options?.forceCreate) {
      return { mediaItemId: existingMediaItemId };
    }

    const currentUploadGeneration = resolveSessionSubsplashUploadGeneration(
      sessionSubsplashUploadGeneration,
      sermon.subsplashUploadGeneration
    );
    const uploadGeneration = options?.forceCreate
      ? getNextPublishGeneration(currentUploadGeneration)
    ...
    if (options?.forceCreate) {
      ...
      setSessionSubsplashUploadGeneration(uploadGeneration);
      setSeriesPublished(false);
    }
    ...
    await updateDoc(sermonRef, {
      ...
      approverId: user?.uid,
      ...(options?.forceCreate ? { subsplashUploadGeneration: uploadGeneration } : {}),
    });
    setSessionSubsplashMediaItemId(response.id);
    return { mediaItemId: response.id };
  }, [sermon, sessionSubsplashMediaItemId, sessionSubsplashUploadGeneration, user?.uid]);
  ...
  const uploadToSubsplash = useCallback(async (
    ...
    try {
      ...
      const sermonRef = doc(firestore, 'sermons', sermon.id).withConverter(sermonConverter);
      let id = resolveSessionSubsplashMediaItemId(
      ...
      const runAddToList = (activeMediaItemId: string) => addToList({
        destinationListIds: listsMetadata.map((metadata) => metadata.listId),
        mediaItem: { id: activeMediaItemId, type: 'media-item' },
      ...
      const addToListAttempt = listsMetadata.length === 0
        ? { result: [] as AddToListOutputType, mediaItemId: id }
        : await runWithMissingSubsplashMediaRecovery({
          mediaItemId: id,
          run: runAddToList,
          recreateMediaItem: () => ensureSubsplashMediaItem({ forceCreate: true }),
        });
      const addToListReturn = addToListAttempt.result;
      id = addToListAttempt.mediaItemId;
  ...
  const deleteSubsplashMedia = useCallback(async (): Promise<boolean> => {
    const deleted = await deleteSubsplashMediaAndLocalState({
      sermonId: sermon.id,
      subsplashId: resolveSessionSubsplashMediaItemId(sessionSubsplashMediaItemId, sermon.subsplashId),
      seriesId: sermon.seriesId,
    });
    if (deleted) {
      setSessionSubsplashMediaItemId(null);
      setSessionSubsplashUploadGeneration(getNextPublishGeneration(sermon.subsplashUploadGeneration));
      setSeriesPublished(false);
    }
    return deleted;
  }, [sermon.id, sermon.seriesId, sermon.subsplashId, sermon.subsplashUploadGeneration, sessionSubsplashMediaItemId]);
  ...
  const publishEverywhere = useCallback(async () => {
    ...
    try {
      ...
      >({
        ...
        initialMediaItemId: resolveSessionSubsplashMediaItemId(sessionSubsplashMediaItemId, sermon.subsplashId),
  ...
  const applyAdvancedListChanges = useCallback(async () => {
    ...
    try {
      ...
      let hadError = false;
      let activeSubsplashMediaItemId = resolveSessionSubsplashMediaItemId(
        sessionSubsplashMediaItemId,
        sermon.subsplashId
      );
      ...
      if (selectedListsToPublish.length > 0) {
        ...
        activeSubsplashMediaItemId = publishResult.mediaItemId ?? activeSubsplashMediaItemId;
      ...
      if (selectedSeriesToPublish) {
        const seriesResult = await runWithDestinationActivity({
          ...
          soundCloudOperation: 'idle',
        }, async () => publishToSeries({
          mediaItemId: activeSubsplashMediaItemId,
          suppressNotice: true,
        }));
        if (seriesResult.status === 'success') {
