...
EXPECTED_BGUTIL_VERSION="1.3.2"
EXPECTED_PROVIDER_IMAGE="docker.io/brainicism/bgutil-ytdlp-pot-provider:1.3.2@sha256:9a96e6385ce1928da87dea07b1cab0413d2cf8c07a3b8a8bd419f53df2c3843c"
...
ssh "$SSH_TARGET" "bash -s -- '${REMOTE_DIR}' '${PROVIDER_SERVICE}' '${EXPECTED_PROVIDER_IMAGE}'" <<'HOST_EOF'
set -euo pipefail
...
run_media_canary() {
  local mode="$1"
  local smoke_url="$2"
  local temp_dir stderr_log stdout_log status media_file
  ...
  if (( status != 0 )); then
    if [[ "$mode" == "public_provider" ]] && grep -Eiq \
      "sign in to confirm (you're|you’re) not a bot|login_required|http error 429|the page needs to be reloaded" \
      "$stderr_log"; then
