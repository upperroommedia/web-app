...
async function handleDelete(seriesSermonSnapshot: QuerySnapshot<DocumentData>, sermonId: string) {
  // Delete
  ...
  await adminFirestore.recursiveDelete(adminFirestore.doc(`sermons/${sermonId}`));
}
...
const sermonWriteTrigger = firestore.document('sermons/{sermonId}').onWrite(async (change, context) => {
  ...
  try {
    const seriesSermonSnapshot = await adminFirestore.collectionGroup('listItems').where('id', '==', sermonId).get();
