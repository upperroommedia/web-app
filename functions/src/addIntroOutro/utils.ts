...
function logFFMPEGVersion(ffmpegStaticPath: string) {
 ...
 export const downloadFiles = async (bucket: Bucket, filePaths: FilePaths, tempFiles: Set<string>): Promise<FilePaths> => {
  ...
  export async function executeWithTimout<T>(asyncFunc: () => Promise<T>, cancelFunc: () => void, delay: number): Promise<T> {
