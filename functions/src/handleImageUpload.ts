...
const adminImageConvertor = {
  toFirestore(image: ImagesType): FirebaseFirestore.DocumentData {
    return image;
  },
  fromFirestore(snapshot: FirebaseFirestore.QueryDocumentSnapshot): ImagesType {
    return snapshot.data() as ImagesType;
  },
};
...
const handleImageUpload = onObjectFinalized(
  ...
  async (storageEvent): Promise<void> => {
    ...
    try {
      ...
      axios(uploadConfig)
        .then((response) => logger.log('Axios Response tests:', response))
