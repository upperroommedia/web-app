...
const fixSermonCounts = https.onCall<FixSermonCountsRequest, Promise<FixSermonCountsResponse>>(
    async (request) => {
        ...
        try {
            logger.info(`${validateOnly ? 'Validating' : 'Fixing'} sermon counts${sermonId ? ` for sermon ${sermonId}` : ' for all sermons'}`);
            ...
            if (sermonId) {
                ...
                if (validateOnly) {
                    ...
                    const wasInconsistent = before.numberOfLists !== after.numberOfLists ||
